...
//=========================== 下面是常用的对时间的操作 ==========================================
    ...
    public static String parseStringDate(String sDate) {
        if (CommonHelper.isEmpty(sDate) || sDate.endsWith("00000000")) {
            return "";
     ...
     * @return 
     */
    public static long second2Millisencond(int second) {
        return second * 1000;
    }
    ...
    /**
     * 计算给定时间的 凌晨（00：00：00） 的毫秒值
     *
     * @param date
     * @return
     */
    public static long calculatorStartTimeOnDay(Date date) {
    ...
    public static void traversalOfXML(Node node, List<String> nodeNames, Map<String, String> resultData) throws ParserConfigurationException, SAXException, IOException {
        ...
        for (int i = 0; i < childNodes.getLength(); i++) {
            ...
            if (nodeNames.contains(childNode.getNodeName())) {
                resultData.put(childNode.getNodeName(), childNode.getNodeValue());
            }
        }
    }
    ...
    public static String getParam(Map<String, String> map, String key) {
        ...
        if (CommonHelper.isEmpty(map)) {

            return "";
        }

        return (CommonHelper.isEmpty(map.get(key))) ? "" : map.get(key);
    }
}
